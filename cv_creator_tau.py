...
Stratify by TAU status using MTL SUVR:
    negative: MTL < mtl_thr        (default 1.28)
    positive: MTL >= mtl_thr
...
# Column names / thresholds for TAU (MTL) mode
SESSION_COL_DEFAULT = "TAU_PET_Session"  # join key to subject folders (unchanged logic)
MTL_COL_DEFAULT     = "MTL"              # MTL SUVR column
MTL_THR_DEFAULT     = 1.28               # tau+: SUVR >= 1.28

# Tau label names (simple and readable)
...
def load_tau_labels(meta_csv: str,
                    subjects: List[str],
                    session_col: str,
                    mtl_col: str,
                    mtl_thr: float) -> Dict[str, str]:
    """
    Return mapping: norm_key(sid) -> tau label in {"positive","negative"} based on MTL SUVR.
      - positive: MTL >= mtl_thr   (default 1.28)
      - negative: MTL <  mtl_thr
    ...
    mtl_c  = get_col(df, mtl_col)
    df[mtl_c] = pd.to_numeric(df[mtl_c], errors="coerce")
    ...
    # NOTE: NaN MTL will evaluate as False in (>= thr) and thus become "negative",
    # mirroring prior behavior where unlabeled rows fell into the lowest bucket.
    matched["__label__"] = np.where(matched[mtl_c] >= mtl_thr, "positive", "negative")
...
def main():
    parser = argparse.ArgumentParser("Make 5 CSVs for 5-fold CV with TAU-stratified splits (MTL SUVR).")
    ...
    # Clear, tau-focused knobs (names reflect what they are)
    ...
    parser.add_argument("--mtl_col", default=MTL_COL_DEFAULT,
                        help="CSV column containing MTL SUVR (default: MTL).")
    parser.add_argument("--mtl_thr", type=float, default=MTL_THR_DEFAULT,
                        help="MTL SUVR threshold: negative < thr, positive >= thr (default: 1.28).")
    ...
    # 2) compute TAU labels from metadata CSV using MTL SUVR threshold
    label_map = load_tau_labels(
        args.meta_csv, subjects,
        session_col=args.session_col,
        mtl_col=args.mtl_col,
        mtl_thr=args.mtl_thr
    )
    ...
    if dropped:
        print(f"[WARN] {len(dropped)} subjects lack a tau label and will be excluded from CV.")
        print("       Examples:", dropped[:10])

    # 3) bucket by tau label
    ...
    print("Tau (MTL) counts among included subjects:", total_counts)
    ...
    for f in range(k):
        ...
        print(f"         tau dist train={dist(train_set)} val={dist(val_list)} test={dist(test_list)}")
