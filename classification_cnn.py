...
def braak_stage(row, thr=1.2):
    """Return 0,1,2,3 Braak stage as before."""
...
class CNN6(nn.Module):
    def __init__(self, ncls=2):  # now binary
        super().__init__()
        def blk(cin, cout):
...
# --- binary AUC ---
def binary_auc_safe(y_true, proba_pos):
    # y_true in {0,1}, proba_pos = P(class=1)
...
def run_fold(df, tr_idx, te_idx, path_col, size, device, epochs=70, bs=2):
    ...
    auc = binary_auc_safe(y, prob_pos)
...
def main():
    ...
    print(f"== {args.modality} (binary: {0,1}->{0}, {2,3}->{1}) ==")
