...
def main():
    ...
    for f in folds: print("FOLD:",f)
    subj = collect_subjects_from_folds(folds)
    sids=sorted(subj.keys())
    labels=load_amyloid_labels(args.meta_csv,sids,args.session_col,args.centiloid_col,args.centiloid_thr)
    keep=[s for s in sids if norm_key(s) in labels]
    drop=[s for s in sids if norm_key(s) not in labels]
    if drop: print(f"[WARN] unlabeled excluded: {len(drop)} e.g. {drop[:6]}")
    if not keep: raise RuntimeError("No labeled subjects after join.")
    y=np.array([labels[norm_key(s)] for s in keep],dtype=int)
    print("[LABELS] 0/1:",dict(Counter(y)))

    manifest=pd.DataFrame([{"subject":s,"label":int(labels[norm_key(s)]),**subj[s]} for s in keep]).sort_values("subject")
